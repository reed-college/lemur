...
def login():
    if request.method == 'POST':
        err_msg = u.check_existence(request.form, 'username')
        if err_msg != '':
            return render_template('login.html')
        user = u.get_user(request.form['username'])
...
@u.permission_required(m.Permission.ADMIN)
def admin_home():
    return render_template('admin_home.html')


@app.route('/superadmin_home')
@u.permission_required(m.Permission.SUPERADMIN)
def superadmin_home():
...
@u.permission_required(m.Permission.DATA_ENTRY)
def student_select_lab():
    labs_query = u.get_available_labs_for_user(current_user)
    lab_list = u.serialize_lab_list(labs_query)
...
def student_data_entry(lab_id):
    ...
    experiments_query = u.get_experiments_for_lab(lab_id)

    # check if the lab exists
    if not u.lab_exists(lab_id):
        err_msg = 'The lab with id: {} doesn\'t exist'.format(lab_id)
        return u.err_html(err_msg)

    # Convert queries into JSON format
    experiments = u.serialize_experiment_list(experiments_query)
    lab_info = u.serialize_lab_list([lab_query])
...
def _student_receive_data():
    ...
    err_msg = u.check_existence(jsonData, 'observationsGroupByStudent')
    if err_msg != '':
        return u.err_json(err_msg)
    err_msg = u.add_observations_sent_by_students(jsonData['observationsGroupByStudent'])
    if err_msg != '':
        return u.err_json(err_msg)
    return u.normal_json(jsonData)
...
@u.permission_required(m.Permission.LAB_SETUP)
def admin_create_and_manage_lab():
    current_labs = u.find_all_labs(current_user)
    class_ids = u.get_class_id_list(current_user)
    # If data sent by client is in correct format redirect to lab setup page
    # with user's entered data
    if request.method == 'POST':
        lab_info, err_msg = u.pack_labinfo_sent_from_client(request.form)
        if err_msg != '':
            return u.err_json(err_msg)
...
def admin_modify_lab(lab_id):
    ...
    if not u.lab_exists(lab_id):
        err_msg = ('The lab doesn\'t exist.'
                   'All the existing labs are: {}'.format(', '.join([lab.id for lab in u.get_all_lab()])))
        return u.err_html(err_msg)
    # get info to send to template
    class_ids = u.get_class_id_list(current_user)
    experiments = u.serialize_experiment_list(experiments_query)
    lab_info = u.serialize_lab_list([lab_query])[0]
...
def _admin_duplicate_lab():
    ...
    jsonData = request.get_json()
    # check data format
    err_msg = u.check_existence(jsonData, 'labId')
    if err_msg != '':
        return u.err_json(err_msg)
    u.duplicate_lab(jsonData['labId'])
    return u.normal_json(jsonData)
...
def _admin_change_lab_status(new_status):
    ...
    if err_msg != '':
        return u.err_json(err_msg)
    u.change_lab_status(jsonData['labId'], new_status)
    return u.normal_json(jsonData)
...
@u.permission_required(m.Permission.DATA_EDIT)
def admin_select_lab_for_data():
    # Get the list of labs with basic informati on according to
    # the current user's role
    lab_list = u.find_lab_list_for_user(current_user)
...
def admin_edit_data(lab_ids):
    ...
    (observations, observations_by_student, _,
        err_msg, undownloadable_labs) = u.find_all_observations_for_labs(lab_ids)
    if err_msg == '':
...
def _admin_change_data():
    ...
    jsonData = request.get_json()
    err_msg = u.check_existence(jsonData, 'oldObservationIdsList',
                                'newObservationList')
    if err_msg != '':
        return u.err_json(err_msg)
    ...
    u.delete_observation(jsonData['oldObservationIdsList'])
    err_msg = u.add_observation(jsonData['newObservationList'])
    if err_msg != '':
            return u.err_json(err_msg)
...
def _admin_delete_data():
    jsonData = request.get_json()
    err_msg = u.check_existence(jsonData, 'observationIdsToBeRemoved')
    if err_msg != '':
        return u.err_json(err_msg)
    ...
    if not(isinstance(jsonData['observationIdsToBeRemoved'], list)):
        err_msg = 'the value should be a list'
        return u.err_json(err_msg)
    u.delete_observation(jsonData['observationIdsToBeRemoved'])
    return u.normal_json(jsonData)
...
def _admin_download_data(lab_ids):
    ...
    (_, observations_by_student, experiment_names, err_msg, _) = u.find_all_observations_for_labs(lab_ids)
    if err_msg != '':
        return u.err_html(err_msg)
    else:
        # Format download is located in utility.py
        response = u.format_download(observations_by_student,
                                     experiment_names,
                                     lab_ids)
        return response
...
@u.permission_required(m.Permission.USER_MANAGE)
def superadmin_create_and_manage_user():
    err_msg = ''
    if request.method == 'POST':
        err_msg = u.add_user(request.form)
    if err_msg != '':
        return u.err_json(err_msg)
    # Get existing admins and class ids to send to the template
    user_list = u.serialize_user_list(u.get_all_user())
    class_ids = [c.id for c in u.get_all_class()]
...
def _superadmin_change_user_info():
    ...
    err_msg = u.check_existence(jsonData, 'classIds', 'username', 'role')
    if err_msg != '':
        return u.err_json(err_msg)
    u.change_user_info(jsonData['username'], jsonData['role'], jsonData['classIds'])
    return u.normal_json(jsonData)
...
def _superadmin_delete_user():
    jsonData = request.get_json()
    # Check the correctness of data format
    err_msg = u.check_existence(jsonData, 'userIdToBeRemoved')
    if err_msg != '':
        return u.err_json(err_msg)
    u.delete_user(jsonData['userIdToBeRemoved'])
    return u.normal_json(jsonData)
...
def _superadmin_update_info_from_iris():
    jsonData = request.get_json()
    err_msg = u.check_existence(jsonData, 'message')
    if err_msg != '':
        return u.err_json(err_msg)
    ...
    if jsonData['message'] == 'update classes':
        ...
        err_msg = u.check_existence(class_data[0], 'subject', 'course_number',
                                    'term_code', 'instructors')
        if err_msg != '':
            return u.err_json(err_msg)
        warning_msg = u.populate_db_with_classes_and_professors(class_data)
        return u.normal_json(warning_msg)
        ...
        err_msg = u.check_existence(registration_data[0], 'subject',
                                    'course_number', 'term_code', 'user_name')
        if err_msg != '':
            return u.err_json(err_msg)
        warning_msg = u.update_users_by_data_from_iris(registration_data)
        return u.normal_json(warning_msg)
    else:
...
@u.permission_required(m.Permission.LAB_MANAGE)
def superadmin_create_and_manage_class():
    if request.method == 'POST':
        err_msg = u.add_class(request.form)
        if err_msg != '':
            return u.err_json(err_msg)
    # query professors' names and current classes from the database and
    # convert them into JSON format
    classes_query = u.get_all_class()
    all_users = u.get_all_user()
    current_classes = u.serialize_class_list(classes_query)
...
def _superadmin_delete_class():
    jsonData = request.get_json()
    err_msg = u.check_existence(jsonData, 'classIdToBeRemoved')
    if err_msg != '':
        return u.err_json(err_msg)
    u.delete_class(jsonData['classIdToBeRemoved'])
    return u.normal_json(jsonData)
...
def _superadmin_modify_class():
    ...
    jsonData = request.get_json()
    err_msg = u.check_existence(jsonData, 'classId', 'studentUserNames',
                                'professorUserNames')
    if err_msg != '':
        return u.err_json(err_msg)
    err_msg = u.change_class_users(jsonData['classId'],
