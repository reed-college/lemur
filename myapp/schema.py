...
import enum
#Notes: if the schema is changed, you need to change __tablename__ to create a new schema and clear the old one
...
class Lab_info(Base):
    ...
    lab_status = Column(Enum('Activated', 'Downloaded', 'Unactivated', name='lab_status'))

    # one-to-many: a lab can have multiple rows
    lab_rows = relationship('Lab_rows',back_populates='lab',cascade="all, delete, delete-orphan")
    ...
    def __repr__(self):
        ...
        formatted = tpl.format(lab_id=self.lab_id, lab_name=self.lab_name, class_name=self.class_name, prof_name=self.prof_name, lab_desc=self.lab_desc, lab_status=self.lab_status)
        return formatted  
...
class Lab_rows(Base):
    ...
    row_datas = relationship('Lab_data',back_populates='row',cascade="all, delete, delete-orphan")
...
class Lab_data(Base,DateTimeMixin):
    ...
    data_id = Column(String(128),nullable=False,unique=True,primary_key=True) 
    ...
    def __repr__(self):
        ...
        formatted = tpl.format(row_id=self.row_id, data_id=self.data_id, row_data=self.row_data)
        return formatted
